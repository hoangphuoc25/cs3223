 ...
 */
Bucket
_hash_hashkey2bucket(uint32 hashkey, uint32 *yettosplit,
					 uint32 highmask, uint32 lowmask)
{
	Bucket		bucket;
	uint32		num_bucket;
	uint32		whichArrayElement;
	uint32		whichBit;
	
	bucket = hashkey & lowmask;
	num_bucket = lowmask + 1;
	
	whichArrayElement = (bucket/32);
	whichBit = (bucket%32);
		
	if((yettosplit[whichArrayElement] & (1 << whichBit)) == 0) //check if the bucket has been split
...
/*
 * CS3223
 * logBase2 -- returns the integer part of lg2(num)
 */
uint32 logBase2(uint32 num)
{
	uint32		i = 0;
