...
static bool _hash_alloc_buckets(Relation rel, BlockNumber firstblock,
					uint32 nblocks);
/* CS3223 - replace uint32 maxbucket with YETTOSPLITBKTS* yettosplit */
...
_hash_metapinit(Relation rel, double num_tuples, ForkNumber forkNum)
{
	 ...
	 */
	metap->hashm_lowmask = num_buckets - 1;
	metap->hashm_highmask = (num_buckets << 1) - 1;
	
	/* CS3223 - Initialize newly added variables */
	metap->numBktInUse = num_buckets;	
	uint32 temp1[ARRAY_SIZE];
	int k;
	for(k = 0; k<(num_buckets/32); k++)
	{
		temp1[k] = ((uint32) 0xFFFFFFFF);
	}
	temp1[(num_buckets/32)] = (1 << (num_buckets%32)) -1;	
	metap->yetToSplitBkts = temp1; 	
	uint32 temp2[ARRAY_SIZE];
	metap->ovflBkts = temp2;
...
_hash_expandtable(Relation rel, Buffer metabuf)
{
	...
	/* CS3223 */
	uint32		case1Candidates; // Set bits represent buckets that satisfy conditions in case 1 of OFLHI 
	uint32		*yettosplit;
	uint32 		*ovflbkts;
	uint32 i, j;
	uint32 num_bucket; //number of buckets at the beginning of a round
	uint32 maxArrayID; //max index of valid array element
	uint32 case3 = 0; //a flag indicate whether case 3 has been entered or not (0-no, 1-yes)
	...
	if (metap->hashm_ntuples <=
		(double) metap->hashm_ffactor * (metap->numBktInUse))
		goto fail;

	/*
	 * Can't split anymore if numBktInUse has reached its maximum possible
	 * value.
	 *
	 * Ideally we'd allow bucket numbers up to UINT_MAX-1 (no higher because
	 * the calculation numBktInUse mustn't overflow).  Currently we restrict
	 ...
	 * _hash_metapinit().
	 */
	if ((metap->numBktInUse - 1) >= (uint32) 0x7FFFFFFE) /* CS3223 */
		goto fail;
	...
	/* CS3223 
	 * Determine which bucket to split by OFLHI
	 */	
	highmask = metap->hashm_highmask;
	lowmask = metap->hashm_lowmask;
	num_bucket = lowmask + 1;
	yettosplit = metap->yetToSplitBkts;
	ovflbkts = metap->ovflBkts;
	maxArrayID = num_bucket/32;
	
OFLHI:
	for(i = 0, j = -1; i<=maxArrayID; i++)
	{
		case1Candidates = (yettosplit[i] & ovflbkts[i]); 
		
		if((j == -1) && (yettosplit[i] != 0))//there is a yet-to-be-split bucket in this group of 32 buckets
		{
			j = i; //j is updated at most once in this loop
		}
		if(case1Candidates != 0) //found bucket satisfies case 1 condition
			break;
	}
	
	if(case1Candidates != 0) //case 1
	{
		old_bucket = (logBase2(case1Candidates) + i*32); //choose the left-most set bit in the group
	}
	else if(j != -1) //case 2
	{
		
		old_bucket = (logBase2(yettosplit[j]) + j*32); //choose the left-most set bit in the group
	}
	else //case 3
	{
		case3 = 1;
		...
		num_bucket = lowmask + 1;
		maxArrayID = num_bucket/32;
		// every bucket in split points 0,..,i+1 is considered to be yet-to-be-split in the new round
		// thus, turn on all the bits correspond to those bucket
		uint32 k;
		for(k = 0; k < maxArrayID; k++)
		{
			yettosplit[k] = ((uint32) 0xFFFFFFFF); 
		}
		yettosplit[maxArrayID] = (1 << (num_bucket % 32)) - 1;
		goto OFLHI; //go back to decide which bucket to be split by OFLHI
	}
	
	//Map old bucket to its split image
	//For instance, old_bucket 0010 is mapped to new_bucket 1010
	new_bucket = (old_bucket | (lowmask ^ highmask)); 
	...
	/*
	 * If the split point is increasing,
	 * we need to allocate a new batch of bucket pages.
	 */
	...
	if (new_bucket > metap->hashm_highmask) 
	{
		...
		metap->hashm_highmask = (metap->hashm_highmask << 1) | 1; /* CS3223 */
	}

	/*
	 * If the split point is increasing,
	 * we need to adjust the hashm_spares[] array and
	...
	if (spare_ndx > metap->hashm_ovflpoint)
	{
		...
		/* CS3223 - Update metap->yetToSplitBkts (case 1 and case 2 only)
		 * in case 3, yetToSplitBkts has already been updated
		 */
		if(case3 == 0){
			num_bucket = metap->hashm_lowmask + 1; 
			maxArrayID = num_bucket/32;
			uint32 k;
			for(k = 0; k < maxArrayID; k++)
			{
				metap->yetToSplitBkts[k] = ((uint32) 0xFFFFFFFF); 
			}
			metap->yetToSplitBkts[maxArrayID] = (1 << (num_bucket % 32)) - 1;
		}
	}
	
	/* CS3223 - Update newly added variables */
	metap->yetToSplitBkts[old_bucket/32] = (metap->yetToSplitBkts[old_bucket/32] ^ (1 << (old_bucket % 32)));
	metap->numBktInUse++; 
	...
	/* CS3223 - save yetToSplitBkts in metap to pass it to _hash_splitbucket*/
	uint32 temp[ARRAY_SIZE];
	int k;
	for(k = 0; k <= maxArrayID; k++)
	{
		temp[k] = metap->yetToSplitBkts[k];
	}	
	yettosplit = temp;
	...
	/* CS3223 - replace parameter maxbucket with yettosplit*/
	_hash_splitbucket(rel, metabuf, old_bucket, new_bucket,
					  start_oblkno, start_nblkno,
					  yettosplit, highmask, lowmask); 
				  ...
				  uint32 lowmask)
{
	...
	for (;;)
	{
			 ...
			 ooffnum = OffsetNumberNext(ooffnum))
		{
			...
			/* CS3223 - replace maxbucket with yettosplit */
