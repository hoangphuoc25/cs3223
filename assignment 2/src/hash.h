...
//#include "../backend/access/hash/hashutil.c"




/* CS3223 - new data types 
 * For OVFLBKTS and YETTOSPLITBKTS,
 * each bit represents a bucket,
 * nth bit from the right in mth array element corresponds to a bucket with bkt # (m*32+n).
 * 0-based indexing from right to left in each array element.
 */
#define ARRAY_SIZE (1 << 27)
//typedef uint32 OVFLBKTS[ARRAY_SIZE];
//typedef uint32 YETTOSPLITBKTS[ARRAY_SIZE];
...
//	uint32		hashm_maxbucket;	/* ID of maximum bucket in use */ CS3223 - Remove this variable as it is not needed
	...
	/* CS3223 */
	uint32 *ovflBkts; //Set bits represent buckets with overflow chains 
	uint32 *yetToSplitBkts; //Set bits represent buckets that have not been split yet in the current round
	uint32 numBktInUse; //# of buckets in use which is incremented each time a bucket is split (i.e, # of buckets at the start of one round + split images (if any))
} HashMetaPageData;
...
/* CS3223
 * Replace parameter uint32 maxbucket in _hash_hashkey2bucket 
 * with YETTOSPLITBKTS* yettosplit and change the function body 
 * Add one new function logBase2
 */
extern Bucket _hash_hashkey2bucket(uint32 hashkey, uint32 *yettosplit,
					 uint32 highmask, uint32 lowmask); 
extern uint32 logBase2(uint32 num); 
