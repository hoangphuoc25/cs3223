...
hashbulkdelete(PG_FUNCTION_ARGS)
{
	...
	/* determine orig_maxbucket using our structures */
	orig_maxbucket = metap->hashm_lowmask;
...
loop_top:
	...
	/*CS3223*/ //??
	if (cur_maxbucket != metap->hashm_lowmask)
	{
		/* There's been a split, so process the additional bucket(s) */
		cur_maxbucket = metap->hashm_lowmask;		
		memcpy(&local_metapage, metap, sizeof(local_metapage));
		_hash_relbuf(rel, metabuf);
		goto loop_top;
	}
	...
	if (//orig_maxbucket == metap->hashm_maxbucket &&  /*CS3223*/
		orig_ntuples == metap->hashm_ntuples)
	{
		/*
